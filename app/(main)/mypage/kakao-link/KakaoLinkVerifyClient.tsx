    ...
    <section className="grid gap-4 xl:grid-cols-[0.95fr_1.05fr]">
      <div className="rounded-lg border border-yellow-100 bg-yellow-50/45 p-5">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h2 className="text-base font-semibold text-slate-950">카카오 채널에서 먼저 시작</h2>
            <p className="mt-1 text-sm leading-6 text-slate-600">
              카카오톡 채널에 <span className="font-semibold text-slate-900">연동 {userEmail}</span> 형식으로 입력하면 10분 동안 사용할 수 있는 6자리 코드가 발급됩니다.
            </p>
          </div>
        </div>
        ...
        <div className="mt-4 grid gap-2 text-sm text-slate-600">
          ...
          <Step icon={<Link2 className="h-4 w-4" />} text="웹에서 로그인한 계정과 코드의 계정이 같을 때만 연결됩니다." />
      ...
      <form onSubmit={onSubmit} className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">카카오 채널에서 받은 숫자 6자리를 입력해 주세요.</p>
        </div>
