...
export default async function EmploymentCertificatePage() {
  ...
  const [data, profile] = await Promise.all([getPersonalDashboard(user), getEmployeeProfileDetails(user.id)]);
  const displayName = profile.displayName || data.user.name;
    ...
    <div className="mx-auto max-w-[760px] space-y-4 print:max-w-none print:space-y-0">
      <div className="print:hidden flex items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">공식 증명서 양식으로 출력합니다.</p>
        </div>
      ...
      <section className="bg-white px-8 py-8 shadow-sm ring-1 ring-slate-200 print:px-0 print:py-0 print:shadow-none print:ring-0">
        ...
        <table className="mt-20 w-full border-collapse border border-black text-[15px] text-black">
          <tbody>
            ...
            <tr>
              ...
              <Th>직책</Th>
              <Td>{data.profile.teamHeadName === displayName ? '팀장' : '-'}</Td>
            </tr>
            <tr>
              <Th>재직기간</Th>
            ...
            <tr>
              <Th>용도</Th>
              <Td colSpan={3}>회사 제출</Td>
            </tr>
          </tbody>
        </table>

        <div className="border-x border-b border-black px-8 py-10 text-center text-2xl font-semibold text-black">
          상기 사실을 증명함.
        </div>
        ...
        <div className="mt-3 flex items-end justify-between gap-6 text-xs text-black">
          <div className="space-y-1">
            ...
            <div>- 회사 인사/총무 담당 부서에 발급번호로 확인 가능합니다.</div>
            <div>- 주민등록번호는 개인정보 보호를 위해 마스킹 처리됩니다.</div>
          </div>
