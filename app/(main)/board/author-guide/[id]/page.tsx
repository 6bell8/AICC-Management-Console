...
export default function AuthorGuideDetailPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-4xl space-y-4">
      <div className="flex items-center justify-between gap-3"><h1 className="flex min-w-0 items-center gap-2 text-xl font-semibold sm:text-2xl"><BookOpen className="h-5 w-5 text-sky-600" />가이드 상세<DetailTimestampBadge createdAt={guide.createdAt} updatedAt={guide.updatedAt} /></h1><div className="flex gap-2 sm:justify-end"><Button variant="outline" className="h-9 w-9 p-0" onClick={() => router.push('/board/author-guide')} aria-label="저작가이드 목록" title="저작가이드 목록"><ClipboardList className="h-4 w-4 shrink-0" /></Button><Button variant="outline" className="h-9 w-9 p-0" onClick={() => router.back()} aria-label="뒤로가기" title="뒤로가기"><Undo2 className="h-4 w-4 shrink-0" /></Button></div></div>
      <Separator className="bg-slate-200" />
      {!canWrite ? <ReadOnlyNotice /> : null}
      <div className="space-y-2"><div className="flex items-center gap-2 text-sm text-slate-600">제목<LastEditorBadge name={guide.lastEditorName} /></div><Input value={title} onChange={(event) => setTitle(event.target.value)} disabled={writeDisabled} className="border-slate-200 bg-white text-slate-900 shadow-sm placeholder:text-slate-400 focus-visible:ring-slate-100 focus-visible:ring-offset-0" /></div>
      ...
      <div className="pt-2">
        <CommentPanel targetType="AUTHOR_GUIDE" targetId={guide.id} />
      </div>
