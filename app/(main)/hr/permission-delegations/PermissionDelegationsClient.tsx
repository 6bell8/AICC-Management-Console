...
import { CalendarDays, Search, ShieldCheck, Trash2, UserCog, X } from 'lucide-react';
...
function emptyDelegationDraft(defaults?: { delegatorUserId?: string; teamId?: string }): DelegationDraft {
...
export default function PermissionDelegationsClient({ initialData, currentUser }: { initialData: SettingsData; currentUser: AuthUser }) {
  ...
  const defaultDraft = () => emptyDelegationDraft(globalAdmin ? undefined : { delegatorUserId: currentUser.id, teamId: initialData.teams[0]?.id });
  const [data, setData] = useState(initialData);
  const [modalOpen, setModalOpen] = useState(false);
  const [draft, setDraft] = useState<DelegationDraft>(defaultDraft);
  const [userSearch, setUserSearch] = useState('');
  const [teamSearch, setTeamSearch] = useState('');
  const [teamFilter, setTeamFilter] = useState(globalAdmin ? 'ALL' : (initialData.teams[0]?.id ?? 'ALL'));
  ...
  const delegationUsers = useMemo(() => {
    ...
    const scopedUsers = data.approvedUsers.filter((user) => {
      ...
      if (teamFilter === 'UNASSIGNED') return !user.teamId;
  ...
  }, [data.approvedUsers, teamFilter, userSearch]);
  ...
  const resetModal = () => {
    ...
    setUserSearch('');
    setTeamSearch('');
    setTeamFilter(globalAdmin ? 'ALL' : (data.teams[0]?.id ?? 'ALL'));
  };
    ...
    <div className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">팀장 부재, 휴가, 출장 기간 동안 필요한 인사 권한을 기간제로 위임합니다.</p>
        </div>
        {canEdit ? (
          <button
            type="button"
      ...
      <section className="grid gap-3 md:grid-cols-3">
        ...
        <SummaryCard icon={<UserCog className="h-4 w-4" />} label="등록 가능 계정" value={`${data.approvedUsers.length}명`} helper="승인된 AICC 계정 기준" />
      </section>
          ...
          <div className="relative z-10 flex max-h-[92vh] w-full max-w-5xl flex-col overflow-hidden rounded-lg border border-slate-200 bg-white shadow-xl">
            <div className="flex items-start justify-between gap-3 border-b border-slate-100 bg-slate-50/70 px-5 py-4">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">사용자와 팀을 선택한 뒤 위임 범위와 기간을 지정합니다.</p>
              </div>
                ...
                <div className="space-y-4">
                  <section className="rounded-lg border border-slate-100 bg-white p-3">
                    ...
                    <div className="mb-3 flex items-center gap-2 overflow-x-auto pb-1">
                      ...
                      {globalAdmin ? <>
                        <TeamFilterChip label="전체" selected={teamFilter === 'ALL'} onClick={() => setTeamFilter('ALL')} />
                        <TeamFilterChip label="팀 미지정" selected={teamFilter === 'UNASSIGNED'} onClick={() => setTeamFilter('UNASSIGNED')} />
                      </> : null}
                      {data.teams.map((team) => (
                        <TeamFilterChip
                          key={team.id}
                          ...
                          onClick={() => {
                            ...
                            setDraft((prev) => ({ ...prev, teamId: team.id }));
                          }}
                        />
                      ))}
                    </div>
                      ...
                      {delegationUsers.map((user) => (
                        <div key={user.id} className="rounded-md border border-slate-100 bg-slate-50/50 p-2">
                          ...
                          <div className={['mt-2 grid gap-1.5', globalAdmin ? 'grid-cols-2' : 'grid-cols-1'].join(' ')}>
                            ...
                            <SelectChipButton label="대리자" selected={draft.delegateeUserId === user.id} onClick={() => setDraft((prev) => ({ ...prev, delegateeUserId: user.id }))} />
                          </div>
                        </div>
                      ))}
                  ...
                  <section className="rounded-lg border border-slate-100 bg-white p-3">
                    ...
                    <div className="grid max-h-48 gap-2 overflow-y-auto pr-1 sm:grid-cols-2">
                      {delegationTeams.map((team) => (
                        <button key={team.id} type="button" onClick={() => setDraft((prev) => ({ ...prev, teamId: team.id }))} className={['rounded-md border p-3 text-left transition', draft.teamId === team.id ? 'border-sky-200 bg-sky-50 text-sky-900' : 'border-slate-100 bg-slate-50/60 text-slate-700 hover:border-sky-100 hover:bg-sky-50/60'].join(' ')}>
...
function TeamFilterChip({ label, selected, onClick }: { label: string; selected: boolean; onClick: () => void }) {
  return (
    <button type="button" onClick={onClick} className={['shrink-0 rounded-full border px-3 py-1.5 text-xs font-semibold transition', selected ? 'border-sky-200 bg-sky-50 text-sky-700 shadow-sm' : 'border-slate-200 bg-white text-slate-500 hover:border-sky-100 hover:bg-sky-50/70 hover:text-sky-700'].join(' ')}>
      {label}
    </button>
  );
}
