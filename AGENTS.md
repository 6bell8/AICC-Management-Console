...
동적노드 가이드는 현재 `master` 버전을 외부 스냅샷보다 우선한다.
...
- `scripts/db/seed-board.cjs`, `docs/db/mysql-schema.sql`, `README.md`처럼 여러 도메인이 섞인 파일은
  전체 파일을 보호하지 않는다. 대신 동적노드 관련 시드·테이블·문서 블록은 현재 구현을 유지하며
  외부 스냅샷의 나머지 변경만 선별 반영한다.
